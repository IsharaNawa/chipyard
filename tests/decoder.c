#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif

/* decoder.c - C port of decoder.cpp (uses jpg.h C API)
 *
 * This file implements JPEG header parsing functions using C file I/O
 * and the Header/ByteArray types from jpg.h.
 */

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <inttypes.h>
#include "jpg.h"
#include "embedded_cat.h"

/* Helper: read a single byte from file, set header->valid=false on EOF/error */
static int read_byte_or_fail(FILE *f, Header *header) {
	int b = fgetc(f);
	if (b == EOF) {
		header->valid = false;
		return -1;
	}
	return b & 0xFF;
}

/* SOF specifies frame type, dimensions, and number of color components */
void readStartOfFrame(FILE* inFile, Header* header) {
	if (header->numComponents != 0) {
		printf("Error - Multiple SOFs detected\n");
		header->valid = false;
		return;
	}

	int hi = read_byte_or_fail(inFile, header);
	if (!header->valid) return;
	int lo = read_byte_or_fail(inFile, header);
	if (!header->valid) return;
	uint length = ((uint)hi << 8) + (uint)lo;

	int precision = read_byte_or_fail(inFile, header);
	if (!header->valid) return;
	if (precision != 8) {
		printf("Error - Invalid precision: %u\n", (unsigned)precision);
		header->valid = false;
		return;
	}

	int hhi = read_byte_or_fail(inFile, header); if (!header->valid) return;
	int hlo = read_byte_or_fail(inFile, header); if (!header->valid) return;
	header->height = ((uint)hhi << 8) + (uint)hlo;
	int whi = read_byte_or_fail(inFile, header); if (!header->valid) return;
	int wlo = read_byte_or_fail(inFile, header); if (!header->valid) return;
	header->width = ((uint)whi << 8) + (uint)wlo;
	if (header->height == 0 || header->width == 0) {
		printf("Error - Invalid dimensions\n");
		header->valid = false;
		return;
	}

	int numComp = read_byte_or_fail(inFile, header); if (!header->valid) return;
	header->numComponents = (byte)numComp;
	if (header->numComponents == 4) {
		printf("Error - CMYK color mode not supported\n");
		header->valid = false;
		return;
	}
	if (header->numComponents == 0) {
		printf("Error - Number of color components must not be zero\n");
		header->valid = false;
		return;
	}

	for (uint i = 0; i < header->numComponents; ++i) {
		int componentID = read_byte_or_fail(inFile, header); if (!header->valid) return;
		if (componentID == 0) {
			header->zeroBased = true;
		}
		if (header->zeroBased) {
			componentID += 1;
		}
		if (componentID == 4 || componentID == 5) {
			printf("Error - YIQ color mode not supported\n");
			header->valid = false;
			return;
		}
		if (componentID == 0 || componentID > 3) {
			printf("Error - Invalid component ID: %u\n", (unsigned)componentID);
			header->valid = false;
			return;
		}
		ColorComponent* component = &header->colorComponents[componentID - 1];
		if (component->used) {
			printf("Error - Duplicate color component ID\n");
			header->valid = false;
			return;
		}
		component->used = true;
		int samplingFactor = read_byte_or_fail(inFile, header); if (!header->valid) return;
		component->horizontalSamplingFactor = (byte)(samplingFactor >> 4);
		component->verticalSamplingFactor = (byte)(samplingFactor & 0x0F);
		int qid = read_byte_or_fail(inFile, header); if (!header->valid) return;
		component->quantizationTableID = (byte)qid;
		if (component->quantizationTableID > 3) {
			printf("Error - Invalid quantization table ID in frame components\n");
			header->valid = false;
			return;
		}
	}
	if (length - 8 - (3 * header->numComponents) != 0) {
		printf("Error - SOF invalid\n");
		header->valid = false;
	}
}

/* DQT contains one or more quantization tables */
void readQuantizationTable(FILE* inFile, Header* header) {
	int hi = read_byte_or_fail(inFile, header); if (!header->valid) return;
	int lo = read_byte_or_fail(inFile, header); if (!header->valid) return;
	int length = ((int)hi << 8) + lo;
	length -= 2;

	while (length > 0) {
		int tableInfo = read_byte_or_fail(inFile, header); if (!header->valid) return;
		length -= 1;
		int tableID = tableInfo & 0x0F;

		if (tableID > 3) {
			printf("Error - Invalid quantization table ID: %u\n", (unsigned)tableID);
			header->valid = false;
			return;
		}
		header->quantizationTables[tableID].set = true;

		if (tableInfo >> 4 != 0) {
			for (uint i = 0; i < 64; ++i) {
				int a = read_byte_or_fail(inFile, header); if (!header->valid) return;
				int b = read_byte_or_fail(inFile, header); if (!header->valid) return;
				header->quantizationTables[tableID].table[i] = ((uint)a << 8) + (uint)b;
			}
			length -= 128;
		}
		else {
			for (uint i = 0; i < 64; ++i) {
				int v = read_byte_or_fail(inFile, header); if (!header->valid) return;
				header->quantizationTables[tableID].table[i] = (uint)v;
			}
			length -= 64;
		}
	}

	if (length != 0) {
		printf("Error - DQT invalid\n");
		header->valid = false;
	}
}

/* DHT contains one or more Huffman tables */
void readHuffmanTable(FILE* inFile, Header* header) {
	int hi = read_byte_or_fail(inFile, header); if (!header->valid) return;
	int lo = read_byte_or_fail(inFile, header); if (!header->valid) return;
	int length = ((int)hi << 8) + lo;
	length -= 2;

	while (length > 0) {
		int tableInfo = read_byte_or_fail(inFile, header); if (!header->valid) return;
		int tableID = tableInfo & 0x0F;
		int ACTable = (tableInfo >> 4) & 0x01;

		if (tableID > 3) {
			printf("Error - Invalid Huffman table ID: %u\n", (unsigned)tableID);
			header->valid = false;
			return;
		}

		HuffmanTable* hTable;
		if (ACTable) {
			hTable = &header->huffmanACTables[tableID];
		}
		else {
			hTable = &header->huffmanDCTables[tableID];
		}
		hTable->set = true;

		hTable->offsets[0] = 0;
		uint allSymbols = 0;
		for (uint i = 1; i <= 16; ++i) {
			int cnt = read_byte_or_fail(inFile, header); if (!header->valid) return;
			allSymbols += (uint)cnt;
			hTable->offsets[i] = (byte)allSymbols;
		}
		if (allSymbols > 162) {
			printf("Error - Too many symbols in Huffman table\n");
			header->valid = false;
			return;
		}

		for (uint i = 0; i < allSymbols; ++i) {
			int sym = read_byte_or_fail(inFile, header); if (!header->valid) return;
			hTable->symbols[i] = (byte)sym;
		}

		length -= 17 + (int)allSymbols;
	}
	if (length != 0) {
		printf("Error - DHT invalid\n");
		header->valid = false;
	}
}

/* SOS contains color component info for the next scan */
void readStartOfScan(FILE* inFile, Header* header) {
	if (header->numComponents == 0) {
		printf("Error - SOS detected before SOF\n");
		header->valid = false;
		return;
	}

	int hi = read_byte_or_fail(inFile, header); if (!header->valid) return;
	int lo = read_byte_or_fail(inFile, header); if (!header->valid) return;
	uint length = ((uint)hi << 8) + (uint)lo;

	for (uint i = 0; i < header->numComponents; ++i) {
		header->colorComponents[i].used = false;
	}

	int numComponents = read_byte_or_fail(inFile, header); if (!header->valid) return;
	for (int i = 0; i < numComponents; ++i) {
		int componentID = read_byte_or_fail(inFile, header); if (!header->valid) return;
		if (header->zeroBased) componentID += 1;
		if (componentID > header->numComponents) {
			printf("Error - Invalid color component ID: %u\n", (unsigned)componentID);
			header->valid = false;
			return;
		}
		ColorComponent* component = &header->colorComponents[componentID - 1];
		if (component->used) {
			printf("Error - Duplicate color component ID: %u\n", (unsigned)componentID);
			header->valid = false;
			return;
		}
		component->used = true;

		int huffmanTableIDs = read_byte_or_fail(inFile, header); if (!header->valid) return;
		component->huffmanDCTableID = (byte)(huffmanTableIDs >> 4);
		component->huffmanACTableID = (byte)(huffmanTableIDs & 0x0F);
		if (component->huffmanDCTableID > 3) {
			printf("Error - Invalid Huffman DC table ID: %u\n", (unsigned)component->huffmanDCTableID);
			header->valid = false;
			return;
		}
		if (component->huffmanACTableID > 3) {
			printf("Error - Invalid Huffman AC table ID: %u\n", (unsigned)component->huffmanACTableID);
			header->valid = false;
			return;
		}
	}

	int s1 = read_byte_or_fail(inFile, header); if (!header->valid) return;
	int s2 = read_byte_or_fail(inFile, header); if (!header->valid) return;
	header->startOfSelection = (byte)s1;
	header->endOfSelection = (byte)s2;
	int sa = read_byte_or_fail(inFile, header); if (!header->valid) return;
	header->successiveApproximationHigh = (byte)(sa >> 4);
	header->successiveApproximationLow = (byte)(sa & 0x0F);

	if (header->startOfSelection != 0 || header->endOfSelection != 63) {
		printf("Error - Invalid spectral selection\n");
		header->valid = false;
		return;
	}
	if (header->successiveApproximationHigh != 0 || header->successiveApproximationLow != 0) {
		printf("Error - Invalid successive approximation\n");
		header->valid = false;
		return;
	}

	if (length - 6 - (2 * numComponents) != 0) {
		printf("Error - SOS invalid\n");
		header->valid = false;
	}
}

/* restart interval is needed to stay synchronized during data scans */
void readRestartInterval(FILE* inFile, Header* header) {
	int hi = read_byte_or_fail(inFile, header); if (!header->valid) return;
	int lo = read_byte_or_fail(inFile, header); if (!header->valid) return;
	uint length = ((uint)hi << 8) + (uint)lo;

	int a = read_byte_or_fail(inFile, header); if (!header->valid) return;
	int b = read_byte_or_fail(inFile, header); if (!header->valid) return;
	header->restartInterval = (byte)(((uint)a << 8) + (uint)b);
	if (length - 4 != 0) {
		printf("Error - DRI invalid\n");
		header->valid = false;
	}
}

/* APPNs simply get skipped based on length */
void readAPPN(FILE* inFile, Header* header) {
	int hi = read_byte_or_fail(inFile, header); if (!header->valid) return;
	int lo = read_byte_or_fail(inFile, header); if (!header->valid) return;
	uint length = ((uint)hi << 8) + (uint)lo;

	for (uint i = 0; i < length - 2; ++i) {
		if (read_byte_or_fail(inFile, header) < 0) return;
	}
}

/* comments simply get skipped based on length */
void readComment(FILE* inFile, Header* header) {
	int hi = read_byte_or_fail(inFile, header); if (!header->valid) return;
	int lo = read_byte_or_fail(inFile, header); if (!header->valid) return;
	uint length = ((uint)hi << 8) + (uint)lo;

	for (uint i = 0; i < length - 2; ++i) {
		if (read_byte_or_fail(inFile, header) < 0) return;
	}
}

Header* readJPG(FILE* inFile) {
	Header* header = (Header*)malloc(sizeof(Header));
	if (header == NULL) return NULL;
	byte last = 0, current = 0;

	header_init(header);

	/* first two bytes must be 0xFF, SOI */
	int v = read_byte_or_fail(inFile, header); if (!header->valid) { return header; }
	last = (byte)v;
	v = read_byte_or_fail(inFile, header); if (!header->valid) { return header; }
	current = (byte)v;
	if (last != 0xFF || current != SOI) {
		header->valid = false;
		return header;
	}
	v = read_byte_or_fail(inFile, header); if (!header->valid) return header; last = (byte)v;
	v = read_byte_or_fail(inFile, header); if (!header->valid) return header; current = (byte)v;

	while (header->valid) {
		if (feof(inFile) || ferror(inFile)) {
			printf("Error - File ended prematurely\n");
			header->valid = false;
			return header;
		}
		if (last != 0xFF) {
			printf("Error - Expected a marker\n");
			header->valid = false;
			return header;
		}

		if (current == SOF0) {
			header->frameType = SOF0;
			readStartOfFrame(inFile, header);
		}
		else if (current == DQT) {
			readQuantizationTable(inFile, header);
		}
		else if (current == DHT) {
			readHuffmanTable(inFile, header);
		}
		else if (current == SOS) {
			readStartOfScan(inFile, header);
			break;
		}
		else if (current == DRI) {
			readRestartInterval(inFile, header);
		}
		else if (current >= APP0 && current <= APP15) {
			readAPPN(inFile, header);
		}
		else if (current == COM) {
			readComment(inFile, header);
		}
		else if ((current >= JPG0 && current <= JPG13) || current == DNL || current == DHP || current == EXP) {
			readComment(inFile, header);
		}
		else if (current == TEM) {
			/* TEM has no size */
		}
		else if (current == 0xFF) {
			int nv = read_byte_or_fail(inFile, header); if (!header->valid) return header; current = (byte)nv;
			continue;
		}
		else if (current == SOI) {
			printf("Error - Embedded JPGs not supported\n");
			header->valid = false;
			return header;
		}
		else if (current == EOI) {
			printf("Error - EOI detected before SOS\n");
			header->valid = false;
			return header;
		}
		else if (current == DAC) {
			printf("Error - Arithmetic Coding mode not supported\n");
			header->valid = false;
			return header;
		}
		else if (current >= SOF0 && current <= SOF15) {
			printf("Error - SOF marker not supported: 0x%02x\n", (unsigned)current);
			header->valid = false;
			return header;
		}
		else if (current >= RST0 && current <= RST7) {
			printf("Error - RSTN detected before SOS\n");
			header->valid = false;
			return header;
		}
		else {
			printf("Error - Unknown marker: 0x%02x\n", (unsigned)current);
			header->valid = false;
			return header;
		}
		int nv = read_byte_or_fail(inFile, header); if (!header->valid) return header; last = (byte)nv;
		nv = read_byte_or_fail(inFile, header); if (!header->valid) return header; current = (byte)nv;
	}

	if (header->valid) {
		int cv = read_byte_or_fail(inFile, header); if (!header->valid) return header; current = (byte)cv;
		while (1) {
			if (feof(inFile) || ferror(inFile)) {
				printf("Error - File ended prematurely\n");
				header->valid = false;
				return header;
			}

			last = current;
			int nv = read_byte_or_fail(inFile, header); if (!header->valid) return header; current = (byte)nv;
			if (last == 0xFF) {
				if (current == EOI) {
					break;
				} else if (current == 0x00) {
					if (byte_array_push(&header->huffmanData, last) != 0) { header->valid = false; return header; }
					nv = read_byte_or_fail(inFile, header); if (!header->valid) return header; current = (byte)nv;
				} else if (current >= RST0 && current <= RST7) {
					nv = read_byte_or_fail(inFile, header); if (!header->valid) return header; current = (byte)nv;
				} else if (current == 0xFF) {
					continue;
				} else {
					printf("Error - Invalid marker during compressed data scan: 0x%02x\n", (unsigned)current);
					header->valid = false;
					return header;
				}
			} else {
				if (byte_array_push(&header->huffmanData, last) != 0) { header->valid = false; return header; }
			}
		}
	}

	if (header->numComponents != 1 && header->numComponents != 3) {
		printf("Error - %u color components given (1 or 3 required)\n", (unsigned)header->numComponents);
		header->valid = false;
		return header;
	}

	if (header->colorComponents[0].horizontalSamplingFactor != 1 || header->colorComponents[0].verticalSamplingFactor != 1) {
		printf("Error - Unsupported sampling factor\n");
		header->valid = false;
		return header;
	}
	if (header->numComponents == 3) {
		if (header->colorComponents[1].horizontalSamplingFactor != 1 || header->colorComponents[1].verticalSamplingFactor != 1) {
			printf("Error - Unsupported sampling factor\n");
			header->valid = false;
			return header;
		}
		if (header->colorComponents[2].horizontalSamplingFactor != 1 || header->colorComponents[2].verticalSamplingFactor != 1) {
			printf("Error - Unsupported sampling factor\n");
			header->valid = false;
			return header;
		}
	}
	for (uint i = 0; i < header->numComponents; ++i) {
		if (header->quantizationTables[header->colorComponents[i].quantizationTableID].set == false) {
			printf("Error - Color component using uninitialized quantization table\n");
			header->valid = false;
			return header;
		}
		if (header->huffmanDCTables[header->colorComponents[i].huffmanDCTableID].set == false) {
			printf("Error - Color component using uninitialized Huffman DC table\n");
			header->valid = false;
			return header;
		}
		if (header->huffmanACTables[header->colorComponents[i].huffmanACTableID].set == false) {
			printf("Error - Color component using uninitialized Huffman AC table\n");
			header->valid = false;
			return header;
		}
	}

	return header;
}

/* print all info extracted from the JPG file */
void printHeader(Header* header) {
	if (header == NULL) return;
	printf("DQT=============\n");
	for (uint i = 0; i < 4; ++i) {
		if (header->quantizationTables[i].set) {
			printf("Table ID: %u\n", (unsigned)i);
			printf("Table Data:");
			for (uint j = 0; j < 64; ++j) {
				if (j % 8 == 0) printf("\n");
				printf("%u ", header->quantizationTables[i].table[j]);
			}
			printf("\n");
		}
	}
	printf("SOF=============\n");
	printf("Frame Type: 0x%02x\n", (unsigned)header->frameType);
	printf("Height: %u\n", header->height);
	printf("Width: %u\n", header->width);
	printf("DHT=============\n");
	printf("DC Tables:\n");
	for (uint i = 0; i < 4; ++i) {
		if (header->huffmanDCTables[i].set) {
			printf("Table ID: %u\n", (unsigned)i);
			printf("Symbols:\n");
			for (uint j = 0; j < 16; ++j) {
				printf("%u: ", (unsigned)(j + 1));
				for (uint k = header->huffmanDCTables[i].offsets[j]; k < header->huffmanDCTables[i].offsets[j + 1]; ++k) {
					unsigned v = (unsigned)header->huffmanDCTables[i].symbols[k];
					printf("%02x ", v);
				}
				printf("\n");
			}
		}
	}
	printf("AC Tables:\n");
	for (uint i = 0; i < 4; ++i) {
		if (header->huffmanACTables[i].set) {
			printf("Table ID: %u\n", (unsigned)i);
			printf("Symbols:\n");
			for (uint j = 0; j < 16; ++j) {
				printf("%u: ", (unsigned)(j + 1));
				for (uint k = header->huffmanACTables[i].offsets[j]; k < header->huffmanACTables[i].offsets[j + 1]; ++k) {
					unsigned v = (unsigned)header->huffmanACTables[i].symbols[k];
					printf("%02x ", v);
				}
				printf("\n");
			}
		}
	}
	printf("SOS=============\n");
	printf("Start of Selection: %u\n", (unsigned)header->startOfSelection);
	printf("End of Selection: %u\n", (unsigned)header->endOfSelection);
	printf("Successive Approximation High: %u\n", (unsigned)header->successiveApproximationHigh);
	printf("Successive Approximation Low: %u\n", (unsigned)header->successiveApproximationLow);
	printf("Restart Interval: %u\n", (unsigned)header->restartInterval);
	printf("Color Components:\n");
	for (uint i = 0; i < header->numComponents; ++i) {
		printf("Component ID: %u\n", (unsigned)(i + 1));
		printf("Horizontal Sampling Factor: %u\n", (unsigned)header->colorComponents[i].horizontalSamplingFactor);
		printf("Vertical Sampling Factor: %u\n", (unsigned)header->colorComponents[i].verticalSamplingFactor);
		printf("Quantization Table ID: %u\n", (unsigned)header->colorComponents[i].quantizationTableID);
		printf("Huffman DC Table ID: %u\n", (unsigned)header->colorComponents[i].huffmanDCTableID);
		printf("Huffman AC Table ID: %u\n", (unsigned)header->colorComponents[i].huffmanACTableID);
	}
	printf("Length of Huffman Data: %zu\n", header->huffmanData.size);
}

int main(void) {

	printf("JPEG Decoder Test\n");

	/* Use embedded image bytes as input instead of reading an external file. */
	FILE *inFile = NULL;

	/* Try to use fmemopen when available (glibc/posix). */
#if defined(__GLIBC__) || defined(_GNU_SOURCE)
	if (embedded_cat_size > 0) {
		inFile = fmemopen((void *)embedded_cat, embedded_cat_size, "rb");
	}
#endif

	/* Fallback: write the embedded bytes into a temporary stream. */
	if (!inFile) {
		inFile = tmpfile();
		if (!inFile) {
			perror("tmpfile");
			return 1;
		}
		if (embedded_cat_size > 0) {
			size_t wrote = fwrite(embedded_cat, 1, embedded_cat_size, inFile);
			if (wrote != embedded_cat_size) {
				fprintf(stderr, "Error writing embedded data to temporary stream\n");
				fclose(inFile);
				return 1;
			}
			rewind(inFile);
		} else {
			fprintf(stderr, "Error - no embedded image data available\n");
			fclose(inFile);
			return 1;
		}
	}

	Header *header = readJPG(inFile);
	fclose(inFile);

	if (header == NULL) {
		printf("Error - Memory error\n");
		return 1;
	}
	if (header->valid == false) {
		printf("Error - Invalid JPG\n");
		header_free(header);
		free(header);
		return 1;
	}

	printHeader(header);

	header_free(header);
	free(header);
	return 0;
}
